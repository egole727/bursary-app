...
def dashboard():
    ...
    allocated_budget = (
        db.session.query(func.sum(Application.amount_allocated))
    ...
    for ward in wards:
        total_ward_allocated = (
            db.session.query(func.sum(Application.amount_allocated))
            .filter(Application.ward_id == ward.id, Application.status == "APPROVED")
            .scalar()
            or 0
        )
...
def applications():
    ...
    if ward_id:
        query = (
            query.join(User, Application.student_id == User.id)
            .join(Profile)
...
def export_report():
    ...
    query = (
        db.session.query(Application, User, AcademicInfo, BursaryProgram, Ward)
        .join(User, Application.student_id == User.id)
        .join(AcademicInfo, User.id == AcademicInfo.user_id)
...
def ward_report(ward_id):
    ...
    approved_count = Application.query.filter_by(
        ward_id=ward_id, status="APPROVED"
    ).count()

    total_allocated = (
        db.session.query(func.coalesce(func.sum(Application.amount_allocated), 0))
        .filter(Application.ward_id == ward_id, Application.status == "APPROVED")
        .scalar()
    )
    ...
    # Get recent applications
    recent_applications = (
        Application.query.filter_by(ward_id=ward_id)
...
def new_ward_admin():
    ...
    return render_template(
        "admin/ward_admin_form.html", form=form, title="New Ward Admin"
    )
...
def edit_ward_admin(id):
    ...
    return render_template(
        "admin/ward_admin_form.html", form=form, ward_admin=ward_admin
    )
