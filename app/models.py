...
class Profile(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(
        db.Integer, db.ForeignKey("user.id", name="fk_profile_user"), unique=True
    )
...
class Application(db.Model):
    ...
    program_id = db.Column(
        db.Integer, db.ForeignKey("bursary_program.id"), nullable=False
    )
    ...
    timeline = db.relationship(
        "ApplicationTimeline", backref="application", lazy="dynamic"
    )
    ...
    def validate_allocation(self, amount):
        ...
        if amount > self.program.amount:
            raise ValueError(
                f"Allocated amount (KES {amount:,.2f}) cannot exceed program budget (KES {self.program.amount:,.2f})"
            )
        ...
        total_allocated = (
            ...
            .filter(
                Application.program_id == self.program_id,
        ...
        if amount > remaining_budget:
            raise ValueError(
                f"Insufficient program budget. Remaining: KES {remaining_budget:,.2f}"
            )
...
class AcademicInfo(db.Model):
    ...
    )  # Optional for primary/secondary students
