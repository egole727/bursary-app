...
class ProfileForm(FlaskForm):
    ...
    def validate_phone_number(self, field):
        ...
        if profile and (
            not current_user.profile or profile.id != current_user.profile.id
        ):
...
class AcademicInfoForm(FlaskForm):
    ...
    # Banking Details
    school_account_number = StringField(
        "School Account Number", validators=[DataRequired()]
    )
    ...
    def validate_student_id(self, field):
        ...
        if academic and (
            not current_user.academic_info
            or academic.id != current_user.academic_info.id
        ):
...
class ApplicationForm(FlaskForm):
    ...
    reason = TextAreaField(
        "Reason for Application", validators=[DataRequired(), Length(min=100, max=1000)]
    )
