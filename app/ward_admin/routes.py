...
def dashboard():
    ...
    stats = {
        ...
        "pending_applications": Application.query.filter_by(
            ward_id=current_user.ward_id, status="PENDING"
        ).count(),
        "approved_applications": Application.query.filter_by(
            ward_id=current_user.ward_id, status="APPROVED"
        ).count(),
    ...
    recent_applications = (
        Application.query.filter_by(ward_id=current_user.ward_id)
...
def review_application(application_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            flash(str(e), "error")
            return redirect(
                url_for("ward_admin.review_application", application_id=application.id)
            )
    ...
    return render_template(
        "ward_admin/review_application.html",
        form=form,
...
def reports():
    ...
    stats = {
        ...
        "rejected_applications": Application.query.filter_by(
            ward_id=current_user.ward_id, status="REJECTED"
        ).count(),
        "pending_applications": Application.query.filter_by(
            ward_id=current_user.ward_id, status="PENDING"
        ).count(),
    ...
    if format == "csv":
        ...
        with pd.ExcelWriter(excel_file, engine="xlsxwriter") as writer:
            ...
            header_format = workbook.add_format(
                {"bold": True, "bg_color": "#4B88B4", "font_color": "white"}
            )
...
def export_report():
    ...
    query = (
        db.session.query(Application, User, AcademicInfo, BursaryProgram)
        ...
        .filter(Application.ward_id == current_user.ward_id)
    )
