...
def send_email(subject, recipient, template, **kwargs):
    ...
    if not sender:
        raise ValueError(
            "Email configuration is incomplete. Please set MAIL_USERNAME or MAIL_DEFAULT_SENDER in .env file"
        )
...
def send_verification_email(user, token):
    ...
    verification_url = url_for(
        "auth.verify_email", token=token, _external=True, _scheme=scheme
    )
    ...
    send_email(
        "Verify Your Email - Bursary Application System",
...
def send_password_reset_email(user, token):
    ...
    reset_url = url_for(
        "auth.reset_password", token=token, _external=True, _scheme=scheme
    )
    ...
    send_email(
        "Reset Your Password - Bursary Application System",
        recipient=user.email,
